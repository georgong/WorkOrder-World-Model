...
export default function Dashboard({ data, onReset }: Props) {
  ...
  return (
    <div className="flex min-h-[calc(100vh-40px)] bg-slate-50 rounded-lg overflow-hidden border border-slate-200 mt-1 shadow-sm">
      {/* Sidebar */}
      <aside className="w-40 bg-white border-r border-slate-200 flex flex-col">
        <div className="p-2 border-b border-slate-100">
          <h2 className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2">
            Navigation
          </h2>
          <div className="space-y-0.5">
            {tabs.map((t) => (
              <button
                key={t.id}
                onClick={() => setTab(t.id)}
                className={`w-full text-left px-2 py-1.5 rounded-md text-[11px] font-medium transition-all duration-200 ${
                  tab === t.id
                    ? "sidebar-link-active"
        ...
        <div className="p-2 mt-auto bg-slate-50 border-t border-slate-200">
          <h3 className="text-[9px] font-semibold text-slate-500 mb-1">
            Schedule Info
          </h3>
          ...
          <button
            onClick={onReset}
            className="mt-2 w-full py-1 px-2 border border-slate-300 rounded-md text-[9px] font-semibold text-slate-600 hover:bg-white hover:text-brand-blue transition-colors"
          >
           Upload New File
          </button>
        </div>
      ...
      {/* Main Content */}
      <main className="flex-1 p-3 overflow-y-auto max-h-[calc(100vh-40px)]">
